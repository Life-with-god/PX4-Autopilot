...
### 5.1 方案 A：git cherry-pick（当 LOCP 改动集中在有限提交时）
...
### 5.2 方案 B：手动补丁应用（当 LOCP 改动散落在多个提交时）
...
#    - updateLOCP() 函数
#    - checkAttitudeRateAnomaly() / checkVelocityRateAnomaly() 等 8 个检测函数
#    - checkOffboardSetpointSanity()（OBS）
#    - checkThrustResponse()（TRD）
#    - 内部状态变量和订阅
#    - 参数注册
```
...
- [ ] LOCP 的 8 个检测函数 + `updateLOCP()` 是否完整移植？
...
- [ ] 全部 63 个 LOCP 参数（`LOCP_EN`、`LOCP_ARD_*`、`LOCP_VRD_*`、`LOCP_PRD_*`、`LOCP_COD_*`、`LOCP_MTO_*`、`LOCP_TRD_*`、`LOCP_OBS_*`、`LOCP_CRASH_*`）是否完整？
- [ ] 所有使能开关默认 = 0（禁用）？（`LOCP_*_EN`）
...
```bash
# 搜索 LOCP 同步代码是否仍在
grep -n "locp_obs_triggered\|locp_trd_triggered\|getLOCP_ARD" src/modules/commander/Commander.cpp
```

### 6.5 `failsafe.cpp` / `failsafe.h`

**检查点**:
- [ ] LOCP 动作是否已按固定动作硬编码（ARD/VRD/PRD/COD/Crash → Disarm，MTO/OBS → 健康则 Land / 不健康则 Disarm）？
- [ ] `checkStateAndMode()` 中 LOCP 的 CHECK_FAILSAFE 调用（各维度 + Crash + TRD 分级）是否完整？
- [ ] 已删除：`fromLOCPActParam()`、`locp_failsafe_action` 枚举、`LOCP_L1/L2/L3_ACT`、`LOCP_OBS_ACT`（确认无残留）
...
| `Action::Descend` 未定义 | 上游删了枚举 | 更新动作硬编码处的枚举使用 |
...
| 场景 | ARD | VRD | PRD | COD | MTO | OBS | 固定动作 |
|------|-----|-----|-----|-----|-----|-----|---------|
| 单 IMU 故障 | ✅ | — | — | — | — | — | Disarm 停桨 |
| EKF 发散(纯) | — | ✅ | ✅ | — | — | — | Disarm 停桨 |
| 电机堵转 | ✅ | — | — | ✅ | — | — | Disarm 停桨 |
| 全系统崩溃 | ✅ | ✅ | ✅ | — | — | — | Disarm 停桨 |
| 仅心跳丢失 | — | — | — | — | ✅ | — | Land 降落 |
| setpoint异常 | — | — | — | — | — | ✅ | 健康→Land / 不健康→Disarm |
| 多维度叠加 | ✅ | — | — | ✅ | — | — | Disarm（最严格动作优先） |
...
| 测试项 | 方法 | 预期结果 |
|--------|------|----------|
| ARD 触发 | 降低 `LOCP_ARD_R_MAX`，剧烈机动 | Disarm 停桨 |
| VRD 触发 | 降低 `LOCP_VRD_AH_MAX` | Disarm 停桨 |
| PRD 触发 | 降低 `LOCP_PRD_ASTD` | Disarm 停桨 |
| COD 触发 | 降低 `LOCP_COD_DELTA_I` | Disarm 停桨 |
| MTO 单触发 | 断开 MAVLink 心跳 | 健康→Land / 不健康→Disarm |
| OBS 触发 | 发送跳变 setpoint | Land 降落 |
| Crash 触发 | 降低阈值后撞击 | Disarm |
| 多维度叠加 | ARD+COD 同时 | Disarm（最严格动作） |
...
```bash
# 首飞保守配置：先只开启 TRD（降落）+ Crash（停桨）
param set LOCP_EN 1
param set LOCP_TRD_EN 1
param set LOCP_CRASH_EN 1
# 其余维度确认无误报后逐项开启（ARD/VRD/OBS 等）
```
